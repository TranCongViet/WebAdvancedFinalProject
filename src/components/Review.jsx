...
export function Review({ reviews, newComment, newRating, setNewComment, setNewRating, handleAddComment }) {
                        ...
                        {reviews.map((reviews, index) => (
                            <li key={index} className="bg-gray-900 p-4 rounded-md">
                                <div className="flex justify-between items-center mt-2 text-gray-400">
                                    <span>{reviews.author}</span>
                                    <span className="text-sm">{reviews.updated_at.toLocaleString()}</span>
                                </div>
                                <div>
                                    <p>{reviews.content}</p>
                                    {
                                        reviews.author == "admin123" && <button className="bg-gray-200 hover:bg-gray-400 text-gray-800 font-bold py-2 px-4 rounded inline-flex items-center">
